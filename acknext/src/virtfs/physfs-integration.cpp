...
ACKNEXT_API_BLOCK
{
	void filesys_addResource(char const *resource, char const *path)
	{
		...
		{
			auto const ec = PHYSFS_getLastErrorCode();
			engine_seterror(ERR_FILESYSTEM, "%s", PHYSFS_getErrorByCode(ec));
		}
	}
}
...
static Sint64 rw_seek(SDL_RWops *c, Sint64 offset, int whence)
{
	...
	else if (whence == RW_SEEK_END)
	{
		...
		if (len < 0)
			return SDL_SetError("Can't seek with _END in a non-sized stream!");
		offset = len - offset;
	}
	file_seek(f, offset);
	return offset;
